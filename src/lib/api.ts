...
import { clonePartTree, ensureUniquePartIds, migrateEmptyBoxRoot } from '../schema';
import { isLocalMode } from './settings.ts';

export const LS_KEY = 'shapecraft.assets.v1';
...
async function apiFetch<T>(path: string, init?: RequestInit): Promise<T | null> {
  // Local-only mode: never touch the backend, go straight to localStorage.
...
/** Notify all asset subscribers (used after an external DB refresh, e.g. ZIP import). */
export function refreshAssets(): void {
  notify();
}
